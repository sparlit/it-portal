...
import { ClipboardList, Plus, Send, CheckCircle2, AlertCircle, Loader2, Download } from 'lucide-react'
import { ReportingService } from '@/lib/reports/ReportingService'
...
export function RequisitionManager() {
  ...
  const exportPDF = () => {
     const headers = ['Number', 'Dept', 'Priority', 'Status', 'Requested By']
     const rows = requisitions.map(r => [r.requisitionNo, r.department, r.priority, r.status, r.requestedBy])
     ReportingService.generatePDF('Procurement Requisition Master List', headers, rows, 'artemis-requisitions')
  }
