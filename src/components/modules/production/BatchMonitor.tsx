...
import { useSensorHeartbeat } from '@/lib/useSensorHeartbeat'

export function BatchMonitor() {
  const [batches, setBatches] = useState<any[]>([])
  const [loading, setLoading] = useState(true)
  const sensors = useSensorHeartbeat()
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4 pt-4 border-t">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold font-mono">{sensors.temp.toFixed(1)}°C</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Activity className="h-4 w-4 text-emerald-500" />
                            <span className="text-xs font-bold font-mono">{sensors.pressure.toFixed(2)} BAR</span>
                        </div>
                        <div className="flex items-center gap-2">
